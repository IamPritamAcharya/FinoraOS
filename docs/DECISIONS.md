...
- **Structured operational logs:** the API uses a small Pino wrapper with request IDs, configured `LOG_LEVEL`, and credential redaction. We log lifecycle events, request completion, gateway choice/completion/fallback, reconciliation metrics, and agent-run lifecycle without logging prompts, financial record bodies, secrets, or authorization headers.
