...
- Ollama/Qwen plus hosted provider abstraction, controlled multi-tool planning, grounded answers, persisted chat, typed proposals, approval/rejection, adjustment records, and reruns. Exception investigation artifacts expose the deterministic amount breakdown, unexplained remainder, qualitative AI explanation, confidence, proposed action, and approval guardrail.
...
- Expired NextAuth refresh sessions display a dedicated FinoraOS re-login screen instead of briefly showing stale workspace chrome or silently restoring the prior identity.
